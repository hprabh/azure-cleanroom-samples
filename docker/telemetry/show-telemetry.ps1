...
if ($telemetryConfigured)
{
    ...
    docker build -f $PSScriptRoot/Dockerfile.otelcollector-local -t otelcollector-local $PSScriptRoot

    $env:TELEMETRY_FOLDER = $telemetryFolder
    $projectName = "telemetry-viewer-$demo"
    docker compose -p $projectName -f $PSScriptRoot/docker-compose.yaml up -d --remove-orphans
