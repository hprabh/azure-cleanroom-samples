...
def ccr_create_joined_dataset_wo_identifiers(
    joined_dataset, joined_dataset_config, modelfile, debug=True
):
...
def execute(query_config):
    ...
    print(
        "Generating aggregated data in "
        + model_output_folder
        + joined_dataset_config["joined_dataset"]
    )
    sandbox_joined_without_key_identifiers = ccr_create_joined_dataset_wo_identifiers(
        sandbox_joined_anon_simplified,
        joined_dataset_config,
        model_output_folder + joined_dataset_config["joined_dataset"],
        True,
    )
